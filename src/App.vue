...
<template>
  <div id="app">
    <div class="w-[calc(100%-240px)] h-[60px] fixed right-0 z-20 bg-[#101010] bg-opacity-80 flex items-center justify-between">
      ...
      <button type="button" @click="!openMenu" :class="openMenu ? 'bg-[#282828]' : 'bg-black'" class="rounded-full hover:bg-[#282828] p-0.5 mr-8 mt-0.5 cursor-pointer">
        <div class="flex items-center">
          <img width="27" class="rounded-full" src="https://cdn.piepme.com/14333/images/piep-MDImwFPf16788523568951678852356895.jpeg" alt="" />
          <div class="text-white text-[14px] ml-1.5 font-semibold">
            Jonas Toi Choi
          </div>
          <ChevronDown v-if="!openMenu" @click="openMenu = true" fillColor="#FFF" :size="30" />
          <ChevronUp v-else @click="openMenu = false" fillColor="#FFF" :size="30" />
        </div>
      </button>

      <span v-if="openMenu" class="fixed rounded-sm shadow-2xl w-[190px] bg-[#282828] top-[52px] right-[35px] p-1 cursor-pointer">
